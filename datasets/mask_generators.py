...
class ReverseCutoutGenerator:
    ...
    def __call__(self, image):
        ...
        x = np.random.randint(width - self.cutout)
        y = np.random.randint(height - self.cutout)
        mask[y:y + self.cutout, x:x + self.cutout, :] = 1
        return mask.astype('uint8')
...
class VideoMaskGenerator:
    def __init__(self):
        ...
        self.generator = MixtureMaskGenerator([
            center, mcar, common, half_01, half_02, half_03, half_04
        ], [2, 2, 2, 1, 1, 1, 1])
...
class OlivettiMaskGenerator:
    def __init__(self):
        left = FixedRectangleGenerator(0, 0, 64, 32)
        bottom = FixedRectangleGenerator(0, 0, 32, 64)
