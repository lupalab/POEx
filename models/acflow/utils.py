    ...
    with tf.variable_scope(name) as scope:
        ...
        if bias:
            biases = variable_on_cpu(
                "biases", [dim_out], tf.constant_initializer(0.))
...
def depool_2x2(input_, stride=2):
    ...
    res = tf.concat(axis=4, values=[
        res, tf.zeros([batch_size, height, stride,
                       width, stride - 1, channels])
    ])
    res = tf.reshape(res, [batch_size, stride *
                           height, stride * width, channels])
...
def as_one_hot(input_, n_indices):
    ...
    indices_input = tf.concat(
        axis=0, values=[indices, tf.reshape(input_, [-1])])
...
def batch_norm(input_,
    ...
    with tf.variable_scope(name):
        ...
        step = variable_on_cpu(
            "step", [], tf.constant_initializer(0.), trainable=False)
        if scale:
            gamma = variable_on_cpu(
                "gamma", [dim], tf.constant_initializer(1.))
